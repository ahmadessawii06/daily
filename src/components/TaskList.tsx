...
export const TaskList: React.FC<TaskListProps> = ({
  ...
  const notDoneCount = tasks.filter((t) => t.status === 'not-done').length;
      ...
      <div className="flex flex-col gap-3 pt-2">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs font-numbers font-bold text-emerald-800 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-500/15 border border-emerald-300 dark:border-emerald-500/30 px-2 py-0.5 rounded-full">
          ...
          <div className="flex items-center gap-2 self-start sm:self-auto">
            ...
            {onOpenExportModal && (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-slate-800 dark:text-zinc-200 bg-slate-100 hover:bg-slate-200 dark:bg-white/[0.06] dark:hover:bg-white/[0.12] border border-slate-200 dark:border-white/[0.1] rounded-xl transition-all cursor-pointer shadow-2xs font-['Alexandria'] min-h-[36px] active:scale-95"
              >
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-emerald-800 dark:text-emerald-300 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 rounded-xl transition-all cursor-pointer shadow-2xs font-['Alexandria'] min-h-[36px] active:scale-95"
            >
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5">
          ...
          {/* Quick Filter tabs - Beautiful unclipped scrollable pills on mobile */}
          <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar p-1 bg-slate-100/90 dark:bg-[#10121a] border border-slate-200 dark:border-white/[0.08] rounded-2xl text-xs font-semibold w-full sm:w-auto shrink-0 select-none">
            
            {/* 1. All */}
            <button
              type="button"
              onClick={() => onFilterChange('all')}
              className={`shrink-0 whitespace-nowrap px-3 sm:px-3.5 py-1.5 rounded-xl transition-all cursor-pointer min-h-[38px] flex items-center justify-center gap-1.5 font-['Alexandria'] active:scale-95 ${
                currentFilter === 'all'
            ...
            >
              ...
              <span className={`text-[11px] font-numbers font-extrabold px-1.5 py-0.5 rounded-md ${
                currentFilter === 'all'
                  ? 'bg-slate-100 text-slate-900'
                  : 'bg-black/5 dark:bg-white/10 text-slate-600 dark:text-zinc-300'
              }`}>
                {tasks.length}
              </span>
            </button>
          ...
          <div className="relative w-full sm:w-52">
            ...
            <input
              ...
              className="bg-white dark:bg-[#10121a] border border-slate-200 dark:border-white/[0.08] rounded-xl ps-8.5 pe-3 py-2 text-xs font-medium text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-zinc-500 focus:outline-none focus:border-emerald-500 shadow-2xs w-full transition-all min-h-[38px]"
            />
          </div>
