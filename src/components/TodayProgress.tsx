...
export const TodayProgress: React.FC<TodayProgressProps> = ({
  ...
  return (
    <div className="bg-gradient-to-b from-[#11131b] to-[#0d0e14] border border-white/[0.08] rounded-2xl p-3.5 sm:p-5 shadow-xl shadow-black/40 relative overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3.5 sm:gap-4">
        ...
        <div className="flex-1 space-y-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-lg bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center text-emerald-400 shrink-0">
            ...
            <span className="text-[11px] sm:text-xs font-mono font-bold text-white tabular-nums bg-white/[0.06] border border-white/[0.08] px-2 py-0.5 rounded-lg">
              {stats.total === 0 
          ...
          <div className="w-full bg-zinc-950 rounded-full h-2 sm:h-2.5 overflow-hidden p-0.5 border border-white/[0.08]">
            <div
        ...
        <div className="grid grid-cols-3 sm:flex items-center gap-1.5 sm:gap-2 w-full sm:w-auto pt-2.5 sm:pt-0 border-t sm:border-t-0 border-white/[0.06]">
          ...
          >
            <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-md bg-emerald-500/20 text-emerald-400 flex items-center justify-center shrink-0">
              <CheckCheck className="w-3 h-3 sm:w-3.5 sm:h-3.5 stroke-[3]" />
            </div>
            <span className="truncate">{lang === 'ar' ? 'مكتملة' : 'Done'}</span>
            <span className="font-mono tabular-nums text-emerald-300 bg-emerald-500/20 px-1 sm:px-1.5 py-0.5 rounded text-[10px] sm:text-[11px]">
              {stats.done}
            </span>
          </button>
          ...
          >
            <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-md bg-amber-500/20 text-amber-400 flex items-center justify-center shrink-0">
              <Clock className="w-3 h-3 sm:w-3.5 sm:h-3.5 stroke-[2.8]" />
            </div>
            <span className="truncate">{lang === 'ar' ? 'انتظار' : 'Pending'}</span>
            <span className="font-mono tabular-nums text-amber-300 bg-amber-500/20 px-1 sm:px-1.5 py-0.5 rounded text-[10px] sm:text-[11px]">
              {stats.pending}
            </span>
          </button>
          ...
          >
            <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-md bg-rose-500/20 text-rose-400 flex items-center justify-center shrink-0">
              <XCircle className="w-3 h-3 sm:w-3.5 sm:h-3.5 stroke-[2.8]" />
            </div>
            <span className="truncate">{lang === 'ar' ? 'غير منجزة' : 'Not Done'}</span>
            <span className="font-mono tabular-nums text-rose-300 bg-rose-500/20 px-1 sm:px-1.5 py-0.5 rounded text-[10px] sm:text-[11px]">
              {stats.notDone}
            </span>
          </button>
